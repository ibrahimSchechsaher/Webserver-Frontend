...
class PixelNorm extends tf.layers.Layer {
  ...
  call(inputs, kwargs) {
    ...
    return (
      inputs *
      tf.rsqrt(
        tf.mean(tf.square(input), (axis = 1), (keepdims = True)) + 0.000000001
      )
    );
  }
...
tf.serialization.registerClass(PixelNorm); // Needed for serialization.

export function pixelnorm() {
  return new PixelNorm();
}
